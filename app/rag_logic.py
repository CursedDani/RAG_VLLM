...
def get_all_documents(db_connection_string: str) -> list:
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("SELECT id, document, embedding FROM langchain_pg_embedding")
            rows = cur.fetchall()
...
def vector_search(query: str, limit: int, db_conn_str: str, vllm_embed: str) -> list:
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("SELECT id, document, embedding FROM langchain_pg_embedding ORDER BY embedding <=> %s DESC LIMIT %s",
