...
if not VLLM_EMBED or not DB_CONNECTION_STRING:
    print("Error: Falta VLLM_EMBED o DB_CONNECTION_STRING en el .env.")
    exit()
...
def create_documents_table_if_not_exists(conn):
    """Crea la tabla 'documents_security' si no existe."""
    with conn.cursor() as cur:
        ...
        """)
        conn.commit()
        print("Tabla 'documents_security' verificada/creada.")
...
def main():
    ...
    for filename in filenames:
        ...
        for i, chunk_text in enumerate(text_chunks):
            ...
            embedding_vector = get_embedding_from_vllm(chunk_text, VLLM_EMBED)
