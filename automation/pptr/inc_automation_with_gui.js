...
function createWindow() {
    mainWindow = new BrowserWindow({
        ...
        title: 'CA Service Desk Manager - Incident Data Extraction Results',
        resizable: true,
        minimizable: true,
        maximizable: true,
    ...
    mainWindow.loadFile('inc_gui.html');

    mainWindow.once('ready-to-show', () => {
        mainWindow.show();

        // Start the automation process after GUI is ready
        console.log('GUI window ready. Starting incident extraction...');
        startAutomation();
    });
...
function startAutomation() {
    ...
    automationProcess = spawn('node', ['inc_extraction.js'], {
        cwd: __dirname,
        ...
        if (mainWindow) {
            if (code !== 0) {
                ...
                mainWindow.webContents.send('automation-error', {
                    message: `Incident extraction failed with exit code ${code}`
                });
            }
        }
    });
