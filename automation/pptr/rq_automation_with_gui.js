...
function createWindow() {
    mainWindow = new BrowserWindow({
        ...
        title: 'CA Service Desk Manager - Request Data Extraction Results',
        resizable: true,
        minimizable: true,
        maximizable: true,
    ...
    mainWindow.loadFile('rq_gui.html');

    mainWindow.once('ready-to-show', () => {
        mainWindow.show();

        // Start the automation process after GUI is ready
        console.log('GUI window ready. Starting request extraction...');
        startAutomation();
    });
...
function startAutomation() {
    ...
    automationProcess = spawn('node', ['rq_extraction.js'], {
        cwd: __dirname,
        ...
        if (mainWindow) {
            if (code !== 0) {
                ...
                mainWindow.webContents.send('automation-error', {
                    message: `Request extraction failed with exit code ${code}`
                });
            }
        }
    });
